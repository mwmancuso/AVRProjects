...
// Used to track string to display over RPM, i.e. duty cycle, "RUN" or "STOP"
char lcd_block_text[7];
...
int main(void) {
	...
	while (1) {
		...
		if (lcd_block == 0) {
			...
			LCD_puts(lcd_block_text);
		}
	}
}

void increment_duty() {
	if (duty_cycle <= 100) duty_cycle += PWM_INCREMENT;
	
	// Forces LCD to display duty cycle for ~3 seconds
	lcd_block = 3;
	
	// Convert duty_cycle to string; put it in global variable
	itoa(duty_cycle, lcd_block_text, 10);
}

void decrement_duty() {
	if (duty_cycle > 0) duty_cycle -= PWM_INCREMENT;
...
void toggle_running() {
	...
	// Forces LCD to display running status for ~3 seconds
	lcd_block = 3;
	
	// Test to see if motor is running; put status in global variable
	if (motor_running) {
		lcd_block_text[0] = 'R';
		lcd_block_text[1] = 'U';
		lcd_block_text[2] = 'N';
...
ISR(TIMER0_COMP_vect) {
	...
	if (pwm_cycle < motor_running*(duty_cycle / PWM_TOTAL_INCREMENTS)) {
	...
	if (pwm_cycle == (PWM_TOTAL_INCREMENTS - 1)) {
		pwm_cycle = 0;
	} else {
		pwm_cycle++;
	}
